...
print("Light GBM is Start")

lgb_train = lgb.Dataset(cv_train_x, cv_train_y)
lgb_valid = lgb.Dataset(cv_test_x, cv_test_y)

model = lgb.train(train_set = lgb_train, valid_sets = [lgb_train, lgb_valid],)

#予測実行
valid_pred = model.predict(cv_test_x)

print(accurate(cv_test_y, valid_pred))
print("Light GBM is complete")
